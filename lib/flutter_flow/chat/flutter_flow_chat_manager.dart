...
class FFChatInfo {
  ...
  final List<UsuarioRecord>? groupMembers;

  UsuarioRecord get currentUser => groupMembers!
      .where((user) => user.reference == currentUserReference)
      .first;
  Map<String, UsuarioRecord> get otherUsers => Map.fromEntries(
        groupMembers!
            .where((user) => user.reference != currentUserReference)
            .map((user) => MapEntry(user.reference.id, user)),
      );
  List<UsuarioRecord> get otherUsersList => otherUsers.values.toList();
  ...
  Stream<FFChatInfo> getChatInfo({
    ...
    return chatStream.asyncMap((chat) async {
      ...
      final groupMembers = await Future.wait(
        userRefs.map((ref) => UsuarioRecord.getDocument(ref).first),
      );
